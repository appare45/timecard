...
// test environment
const demoProjectId = 'DemoProject';
...
describe('ルールテスト', () => {
  test('Firestore', async () => {
    ...
    const AuthenticatedUser1 = testEnvironment.authenticatedContext(
      'Yamada',
      AuthenticatedUserContext
    );
    const AuthenticatedUser2 = testEnvironment.authenticatedContext(
      'Tanaka',
      AuthenticatedUserContext
    );
    // Auth1 -> Auth2
    ...
    test('他のユーザーのデータにはアクセスできない', async () => {
      // Auth1 -> Auth2
      getUser('');
    });
  });
